...
public class CreateWaitPriceJobActivity extends BaseShopActivity {
	...
	public void handle(String user, String msg) {
		...
		if (!price.isEmpty()) {
			...
			System.out.println("Create Job: " + JobType.GETTING_ITEM_DETAILS.name() + ", " + shop + ", " + url + ", "
					+ GoalType.COST_WAITING.name() + ", " + price);
			jobService.createJobWithGoals(JobType.GETTING_ITEM_DETAILS.name(), shop, url, GoalType.SIZE_WAITING.name(),
					price);
			...
			BotUtils.sendMessage(skype, "Please, enter expected price in the format <PRICE=>");
		}
